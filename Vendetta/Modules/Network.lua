...
Ven.recentSystemWhispers = {}
Ven.SyncQueue = {}

local C_MAP =
	{ WARRIOR = 1, PALADIN = 2, HUNTER = 3, ROGUE = 4, PRIEST = 5, SHAMAN = 6, MAGE = 7, WARLOCK = 8, DRUID = 9 }
local C_REV = { "WARRIOR", "PALADIN", "HUNTER", "ROGUE", "PRIEST", "SHAMAN", "MAGE", "WARLOCK", "DRUID" }
local R_MAP =
	{ Human = 1, Dwarf = 2, NightElf = 3, Gnome = 4, Draenei = 5, Orc = 6, Scourge = 7, Tauren = 8, Troll = 9, BloodElf = 10 }
local R_REV = { "Human", "Dwarf", "NightElf", "Gnome", "Draenei", "Orc", "Scourge", "Tauren", "Troll", "BloodElf" }
local F_MAP = { Alliance = 1, Horde = 2 }
local F_REV = { "Alliance", "Horde" }

local function PackPlayer(name, data, isBountyType)
	local fac, lvl, cls, rce =
		F_MAP[data.faction] or 0, tonumber(data.level) or 0, C_MAP[data.classFile] or 0, R_MAP[data.race] or 0
...
local function UnpackPlayer(str)
	...
	cls = tonumber(cls) or 0
	rce = tonumber(rce) or 0
...
ChatFrame_AddMessageEventFilter("CHAT_MSG_SYSTEM", function(self, event, msg)
	...
	for target, timeSent in pairs(Ven.recentSystemWhispers) do
		if now - timeSent < 5 then
			if string.find(string.lower(msg), target) then
				return true
			end
		else
			Ven.recentSystemWhispers[target] = nil
		end
	end
	return false
end)
...
local function HideAutoReplies(self, event, msg, sender)
	...
	if Ven.recentSystemWhispers[s] and (GetTime() - Ven.recentSystemWhispers[s] < 5) then
		return true
	end
	return false
end
ChatFrame_AddMessageEventFilter("CHAT_MSG_AFK", HideAutoReplies)
ChatFrame_AddMessageEventFilter("CHAT_MSG_DND", HideAutoReplies)
...
local function CleanupBounties()
			...
			for owner, data in pairs(owners) do
				...
				if checkWhitelist then
					for wName in string.gmatch(wl, "([^,%s]+)") do
						if wName == string.lower(owner) then
							isWhitelisted = true
							break
						end
					end
				...
				if not isWhitelisted and (now - ts > 86400) then
					owners[owner] = nil
				end
			end
	...
	CleanBoard(Ven.BountyBoard, false)
	CleanBoard(Ven.WantedBoard, true)
end
...
function Ven.QueueSyncMessages(destType)
	...
	local msgsB, msgsW = {}, {}
	table.insert(msgsB, "VEN~CLEAR_BOUNTY")
	table.insert(msgsW, "VEN~CLEAR_WANTED")
	local curB, curW = "", ""
	local _, myClass = UnitClass("player")
	...
	for name, data in pairs(db) do
		if type(data) == "table" then
			if data.isBounty then
				...
				if string.len(curB) + string.len(packed) + 1 > 200 then
					table.insert(msgsB, "VEN~W_SYNC_B~" .. (myClass or "?") .. "~" .. curB)
					curB = packed
				else
					curB = curB == "" and packed or curB .. "^" .. packed
				end
			end
			if data.isWanted then
				local packed = PackPlayer(name, data, false)
				if string.len(curW) + string.len(packed) + 1 > 200 then
					table.insert(msgsW, "VEN~W_SYNC_W~" .. (myClass or "?") .. "~" .. curW)
					curW = packed
				else
					curW = curW == "" and packed or curW .. "^" .. packed
				end
			end
		end
	end
	...
	if destType == "CHANNEL" then
		...
		if id > 0 then
			for _, m in ipairs(msgsB) do
				SendChatMessage(m, "CHANNEL", nil, id)
			end
		end
	else
		for _, m in ipairs(msgsB) do
			table.insert(Ven.SyncQueue, { dest = destType, msg = m })
		end
	end

	local wl = string.lower(db.whitelist or "")
	for wName in string.gmatch(wl, "([^,%s]+)") do
		for _, m in ipairs(msgsW) do
			table.insert(Ven.SyncQueue, { dest = "WHISPER", target = wName, msg = m })
		end
	end
end
...
local function InitNet()
	...
	VendettaDB[rName].NetworkBounties = VendettaDB[rName].NetworkBounties or {}
	Ven.BountyBoard = VendettaDB[rName].NetworkBounties
	VendettaDB[rName].NetworkWanteds = VendettaDB[rName].NetworkWanteds or {}
	Ven.WantedBoard = VendettaDB[rName].NetworkWanteds
	VendettaDB[rName].NetworkCache = VendettaDB[rName].NetworkCache or {}
	Ven.netCache = VendettaDB[rName].NetworkCache
	VendettaDB[rName].SenderClasses = VendettaDB[rName].SenderClasses or {}
	Ven.SenderClasses = VendettaDB[rName].SenderClasses
	VendettaDB[rName].PendingBounties = VendettaDB[rName].PendingBounties or {}
	Ven.PendingBounties = VendettaDB[rName].PendingBounties
	Ven.playerCache = Ven.playerCache or {}
	for k, v in pairs(Ven.netCache) do
		Ven.playerCache[k] = v
	end
	...
	if id == 0 then
		...
		C_Timer.After(2, function()
			RemoveChatWindowChannel(1, chanName)
		end)
	end

	C_Timer.NewTicker(3600, CleanupBounties)
	C_Timer.NewTicker(1800, Ven.ProcessPendingBounties)
	C_Timer.NewTicker(0.8, function()
		if Ven.SyncQueue and #Ven.SyncQueue > 0 then
			...
			elseif q.dest == "ADDON" then
				local func = C_ChatInfo and C_ChatInfo.SendAddonMessage or SendAddonMessage
				if IsInGuild() then
					func(prefix, q.msg, "GUILD")
				end
				if IsInRaid() then
					func(prefix, q.msg, "RAID")
				elseif IsInGroup() then
					func(prefix, q.msg, "PARTY")
				end
			end
		end
	end)
...
f:SetScript("OnEvent", function(self, event, ...)
	if event == "PLAYER_ENTERING_WORLD" then
		C_Timer.After(5, InitNet)
		C_Timer.After(10, Ven.ProcessPendingBounties)
	elseif event == "CHAT_MSG_SYSTEM" then
		local msg = ...
		if string.find(msg, "has come online") then
			if Ven.PendingBounties and #Ven.PendingBounties > 0 then
			...
			if parts[2] == "SEEN_WHISPER" then
				...
				if db[p1] and (db[p1].isWanted or db[p1].isBounty) then
					...
					if timeSinceAlert > 300 or pureZone ~= lastZone then
						...
						local hunterIcon = hunterClass and Ven.GetClassIcon(hunterClass, 12) .. " " or ""
						...
						local targetIcon = cFile and Ven.GetClassIcon(cFile, 12) .. " " or ""
			...
			elseif parts[2] == "ACK_KILL" then
				...
				if Ven.PendingBounties then
					for i = #Ven.PendingBounties, 1, -1 do
						if
							string.lower(Ven.PendingBounties[i].owner) == string.lower(sName)
							and Ven.PendingBounties[i].target == target
						then
							...
							local ownerIcon = oClass and Ven.GetClassIcon(oClass, 12) .. " " or ""
...
function Ven.HandleNetworkMessage(cmd, sender, p1, p2, p3, p4, p5, p6, p7, p8)
	...
	local wl = string.lower(db.whitelist or "")
	for wName in string.gmatch(wl, "([^,%s]+)") do
		if wName == string.lower(sender) then
			isWhitelisted = true
			break
		end
	end
	...
	if cmd == "CLEAR_BOUNTY" then
		...
		if senderClass and senderClass ~= "?" then
			Ven.SenderClasses[sender] = senderClass
		end
		local players = { strsplit("^", packedData) }
		...
		for _, pStr in ipairs(players) do
			...
			if tName and tName ~= "" and tFac ~= myFac then
				Ven.BountyBoard[tName] = Ven.BountyBoard[tName] or {}
				Ven.BountyBoard[tName][sender] = { time = time(), note = tNote }
				Ven.playerCache[tName] = Ven.playerCache[tName] or {}
				if tFac ~= "?" then
					Ven.playerCache[tName].faction = tFac
				end
				if tLvl ~= "?" then
					Ven.playerCache[tName].level = tLvl
				end
				if tCls ~= "?" then
					Ven.playerCache[tName].classFile = tCls
				end
				if tRce ~= "?" then
					Ven.playerCache[tName].race = tRce
				end
				if Ven.netCache then
					Ven.netCache[tName] = Ven.playerCache[tName]
				end
			end
		end
		...
		for _, pStr in ipairs(players) do
			local tName, tFac, tLvl, tCls, tRce, tNote = UnpackPlayer(pStr)
			if tName and tName ~= "" and tFac ~= myFac then
				Ven.WantedBoard[tName] = Ven.WantedBoard[tName] or {}
				Ven.WantedBoard[tName][sender] = { time = time(), note = tNote }
				Ven.playerCache[tName] = Ven.playerCache[tName] or {}
				if tFac ~= "?" then
					Ven.playerCache[tName].faction = tFac
				end
		...
		if p2 ~= myFac then
			...
			Ven.BountyBoard[p1][sender] = { time = time(), note = p6 }
			Ven.playerCache[p1] = Ven.playerCache[p1] or {}
			if p2 and p2 ~= "?" then
				Ven.playerCache[p1].faction = p2
			end
			if p3 and p3 ~= "?" then
				Ven.playerCache[p1].level = tonumber(p3) or p3
			end
			if p4 and p4 ~= "?" then
				Ven.playerCache[p1].classFile = p4
			end
			if p5 and p5 ~= "?" then
				Ven.playerCache[p1].race = p5
			end
			if Ven.netCache then
				Ven.netCache[p1] = Ven.playerCache[p1]
			end
			Ven.SenderClasses = Ven.SenderClasses or {}
			if p7 and p7 ~= "?" then
		...
		if p2 ~= myFac then
			...
			Ven.WantedBoard[p1][sender] = { time = time(), note = p6 }
			Ven.playerCache[p1] = Ven.playerCache[p1] or {}
			if p2 and p2 ~= "?" then
				Ven.playerCache[p1].faction = p2
			end
	...
	if cmd == "WANTED" or cmd == "UNWANTED" then
		...
		for wName in string.gmatch(wl, "([^,%s]+)") do
			if C_ChatInfo and C_ChatInfo.SendAddonMessage then
				C_ChatInfo.SendAddonMessage(prefix, msg, "WHISPER", wName)
			else
				SendAddonMessage(prefix, msg, "WHISPER", wName)
			end
		end
	else
...
function Ven.ManualChannelSync()
	...
	if GetTime() - Ven.lastChannelSync < 3 then
		return
	end
	Ven.lastChannelSync = GetTime()
	Ven.QueueSyncMessages("CHANNEL")
	Ven.QueueSyncMessages("ADDON")
end

C_Timer.After(2, function()
	if Ven.DBFrame then
		Ven.DBFrame:HookScript("OnShow", Ven.ManualChannelSync)
